...
compound_word = "diapers"
A = model.to_single_token(" di")
B = model.to_single_token("apers")
folder = Path("/workspace/data/")/compound_word
data_AB = torch.load(folder/"AB.pt")
data_AX = torch.load(folder/"AX.pt")
data_XB = torch.load(folder/"XB.pt")
...
# %%
nutils.show_df(neuron_df.sort_values("DLAb", ascending=False).head(50))
...
def plot_key_graphs(l, n):
    ...
    fig = px.histogram(vocab_df, x="logit", marginal="rug", title=label, 
                    #    hover_name="token"
                       )
    ...
    px.histogram(temp_df, x="value", color="type", nbins=100, marginal="rug", barmode="overlay", histnorm="percent", title=f"Neuron {label} acts", 
                #  hover_name="prompt"
                 ).show()
    temp_df = plot_dataset_fn(lambda x: x[:, l, n])
    temp_df["A log prob"] = to_numpy(torch.cat([A_lps[0], A_lps[1], A_lps[2]]))
    # px.scatter(temp_df[(temp_df.type=="AB") & (temp_df["A log prob"]>-5)], x="value", y="A log prob", 
    px.scatter(temp_df[(temp_df.type=="AB")], x="value", y="A log prob", 
            #    hover_name="prompt", 
               title=f"Neuron {label} acts vs A LP", opacity=0.4, marginal_x="histogram", marginal_y="histogram", trendline="ols").show()
top_neuron_df = (neuron_df.sort_values("DLAb", ascending=False).head(50))
for i in range(5):
    plot_key_graphs(top_neuron_df.iloc[i].L, top_neuron_df.iloc[i].N)


# %%
batch_AB = data_AB[0:16]
logits, cache_AB = model.run_with_cache(batch_AB)
batch_AX = data_AX[0:16]
logits, cache_AX = model.run_with_cache(batch_AX)
        # neuron_acts.__getattribute__(label)[i:i+batch_size] = einops.rearrange(cache.stack_activation("pre")[:, :, -2, :], "layer batch mlp -> batch layer mlp")
        # log_probs = logits.log_softmax(dim=-1)
        # A_lps.__getattribute__(label)[i:i+batch_size] = log_probs[:, -3, A]
        # B_lps.__getattribute__(label)[i:i+batch_size] = log_probs[:, -2, B]
...
l = 21
ni = 6680
plot_key_graphs(l, ni)
# %%
stack_AB, labels = cache_AB.get_full_resid_decomposition(l, True, False, True, -2, True)
stack_AX, labels = cache_AX.get_full_resid_decomposition(l, True, False, True, -2, True)
win = model.blocks[l].mlp.W_in[:, 6680]
line((stack_AB - stack_AX).mean(dim=1) @ win, x=labels, title="DNA by component per batch")
line((stack_AB.median(dim=1).values - stack_AX.median(dim=1).values) @ win, x=labels, title="DNA by component per batch (median)")
# line(torch.cat([(stack_AB - stack_AX) @ win, (stack_AB - stack_AX).mean(dim=1, keepdim=True) @ win], dim=1).T, x=labels, title="DNA by component per batch", line_labels=list(map(str, range(16)))+["mean"])
# line(, x=labels, title="DNA by component per batch")
# %%
# px.box(np.concatenate([to_numpy(cache_AB["pattern", 15][:, 5, -2]), to_numpy(cache_AX["pattern", 15][:, 5, -2])]), color=0)
for layer, head in [(15, 5), (16, 6), (19, 2), (9, 7), (18, 13), (11, 11), (12, 8), (4, 11)]:
    # px.box(to_numpy(cache_AB["pattern", layer][:, head, -2]), title=f"L{layer}H{head}").show()
    v = cache_AB["value", layer][:, :, head]
    W_O = model.blocks[layer].attn.W_O[head]
    pattern = cache_AB["pattern", layer][:, head, -2]
    decomposed_dna_AB = (v @ W_O @ win) * pattern[:, :]
    
    v = cache_AX["value", layer][:, :, head]
    W_O = model.blocks[layer].attn.W_O[head]
    pattern = cache_AX["pattern", layer][:, head, -2]
    decomposed_dna_AX = (v @ W_O @ win) * pattern[:, :]
    px.box(to_numpy((decomposed_dna_AB - decomposed_dna_AX)), title=f"L{layer}H{head} per position DNA").show()
# px.box(to_numpy(cache_AX["pattern", 15][:, 5, -2])).show()
# %%

# scatter(x=(cache_AB["post", 19][:, -2, :] * (model.blocks[19].mlp.W_out @ win)).mean(0),y= (cache_AX["post", 19][:, -2, :] * (model.blocks[19].mlp.W_out @ win)).mean(0))
line((cache_AB["post", 19][:, -2, :] * (model.blocks[19].mlp.W_out @ win)).mean(0) - (cache_AX["post", 19][:, -2, :] * (model.blocks[19].mlp.W_out @ win)).mean(0))
line((cache_AB["post", 18][:, -2, :] * (model.blocks[18].mlp.W_out @ win)).mean(0) - (cache_AX["post", 18][:, -2, :] * (model.blocks[18].mlp.W_out @ win)).mean(0))
# %%
line([model.blocks[i].mlp.W_out @ win for i in [18, 19, 20]])
line([(model.blocks[i].mlp.W_out)/model.blocks[i].mlp.W_out.norm(dim=-1, keepdim=True) @ win/win.norm() for i in [18, 19, 20]])
# %%
